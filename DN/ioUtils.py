...
class Batch_Feeder:
	...
	'''
	def set_paths(self, idList=None, imageDir=None, gtDir=None):
		self._paths = []
		'''QD
		if self._train:
			for id in idList:
	...
	def next_batch(self):
		...
		if self._train:
			...
			# QD return imageBatch, gtBatch, weightBatch, ssBatch, ssMaskBatch, idBatch
			return imageBatch, gtBatch, weightBatch
		
		else:
			
			for example in self._paths[self._index_in_epoch:min(self._index_in_epoch+self._batchSize, self._numData)]:
				
				image = self.image_scaling(skimage.io.imread(example[1])).astype(float)
				image = scipy.misc.imresize(image, 50)  
				imageBatch.append(self.pad(image))
			...
			# QD return imageBatch, ssBatch, ssMaskBatch, idBatch
			return imageBatch,idBatch
