...
def angularErrorTotal(pred, gt, weight, outputChannels=2):
	with tf.name_scope("angular_error"):
		...
		gt = tf.to_float(tf.reshape(gt, (-1, outputChannels)))
		weight = tf.to_float(tf.reshape(weight, (-1, 1)))
		# QD ss = tf.to_float(tf.reshape(ss, (-1, 1)))
		...
		# QD lossAngleTotal = tf.reduce_sum((tf.abs(errorAngles*errorAngles))*ss*weight)
...
def angularErrorLoss(pred, gt, weight, ss, outputChannels=2):
		...
		return totalLoss
'''
def angularErrorLoss(pred, gt, weight, outputChannels=2):
		lossAngleTotal = angularErrorTotal(pred=pred, gt=gt, weight=weight, outputChannels=outputChannels) / (countTotal()+1)
